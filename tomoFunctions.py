...
def plotLayer(image, vmin = None, vmax = None, area = .6, cmap = 'binary', interpolation = 'none'):
    ...
    area : scalar (0,1]
        % of pixels to maintain within generated bounds (not used if vmin/vmax specified)
    ...
    -------
    AxesImage object
        Image representative of input array.
        
    """
    if vmin is not None and vmax is not None:
        plt.close()
        plt.imshow(image,vmin=vmin,vmax=vmax,cmap=cmap,interpolation= interpolation)
        plt.colorbar()
    else:
        vBounds = _guessVBounds(image, area)
        vMin = vBounds[0]
        vMax = vBounds[1]
        plotLayer(image, vMin, vMax, area, cmap, interpolation)

def reconstruct(sinograms, centers, imageBounds, layers, theta, sigma = .1, ncore = 4, algorithm = 'gridrec'):
    """
    ...
    if len(sinograms.shape) == 4:
        ...
        for el in range(len(sinograms)):
            for x in range(layers[1]-layers[0]):            
                if layers[1]-layers[0] != 1 and x %100 == 0:
                ...
                recon = tomopy.recon(tmp, theta, center = (imageBounds[1] - imageBounds[0])/2.0 + centers[x], algorithm=algorithm,sinogram_order=False,ncore=ncore)
                recons[el][x] += recon[0]
...
class ValHelper:
    ...
    """
    def __init__(self, fig1, fig2, sinograms, imageBounds,layer, layerBounds, theta, sigma, ncore, algorithm, vmin, vmax, cmap, interpolation):
        """
        ...
        self.fig1.imshow(self.recon[0][0], vmin = self.vMin, vmax = self.vMax, cmap = self.cmap)
        self.fig1.cla() #to keep in shape
        self.fig2.imshow(self.recon[0][0], vmin = self.vMin, vmax = self.vMax, cmap = self.cmap)
        ...
        axMax = plt.axes([0.08, 0.05, 0.65, 0.03], facecolor=axcolor)
        axMin = plt.axes([0.08, 0.1, 0.65, 0.03], facecolor=axcolor)
        axCenter = plt.axes([0.08, 0.15, 0.65, 0.03], facecolor=axcolor)
        axLayer = plt.axes([0.08, .2,.09, 0.035])
        axElement = plt.axes([0.08+.09+.11, .20,.09, 0.035])
        axCenterButton = plt.axes([0.08+(.09+.1)*2, .2,.14, 0.035])
    ...
    def largeUpdate(self, val):
        ...
        self.fig1.imshow(self.recon[0][0], vmin = self.vMin, vmax = self.vMax, cmap = self.cmap, interpolation = self.interpolation)
        ...
        self.fig2.imshow(self.recon[0][0], vmin = self.vMin, vmax = self.vMax, cmap = self.cmap, interpolation = self.interpolation)        
        self.versions += [[self.center, self.imageBounds, self.layer, self.vMin, self.vMax, self.element]]
    ...
    def smallUpdate(self, val):
        ...
        self.fig2.imshow(self.recon[0][0], vmin = self.minSlide.val, vmax = self.maxSlide.val, cmap = self.cmap, interpolation = self.interpolation)
        self.minSlide.valtext.set_text(_toSci(self.minSlide.val))
        self.maxSlide.valtext.set_text(_toSci(self.maxSlide.val))
    ...
    def layerSubmit(self, text):
        ...
        try:
            ...
            if(self.layer in range(0,self.layerBounds[1]-self.layerBounds[0])):
                self.fig1.imshow(self.recon[0][0], vmin = self.minSlide.val, vmax = self.maxSlide.val, cmap = self.cmap, interpolation = self.interpolation)
                self.recon = reconstruct(self.sinograms[self.element], self.center, self.imageBounds, self.layer + self.layerBounds[0], self.theta, self.sigma, self.ncore, self.algorithm)
                self.fig2.imshow(self.recon[0][0], vmin = self.vMin, vmax = self.vMax, cmap = self.cmap, interpolation = self.interpolation)
                self.versions += [[self.center, self.imageBounds, self.layer, self.vMin, self.vMax, self.element]]
    ...
    def undo(self, val):
        ...
        if(len(self.versions) >= 3):
            ...
            self.fig1.imshow(tempRecon[0][0], vmin = newVer[3], vmax = newVer[4], cmap = self.cmap, interpolation = self.interpolation)
        else:
            self.fig1.cla()
            self.fig1.set_title("Previous Version")
        if(len(self.versions)>=2):
            ...
            self.fig2.imshow(self.recon[0][0], vmin = oldVer[3], vmax = oldVer[4], cmap = self.cmap, interpolation = self.interpolation)
...
def launchValHelper(sinograms, imageBounds, layer, layerBounds, theta, sigma = .1, ncore = 4, algorithm = 'gridrec', vmin = None, vmax = None, cmap = 'binary', interpolation = 'none'):
    """
    ...
    """
    plt.close()    
    fig1 = plt.axes([.05,.35,.43,.6])
    fig2 = plt.axes([.05+.51,.35,.43,.6])
    return ValHelper(fig1, fig2, sinograms, imageBounds, layer, layerBounds, theta, sigma, ncore, algorithm, vmin, vmax, cmap, interpolation)
...
class Viewer:
    def __init__(self, volume, cmap):
        ...
        ax.volume = volume
        ax.index = 0
        self.bounds = _guessVBounds(volume[len(volume)//2])
        self.cmap = cmap
        ax.imshow(volume[ax.index], vmin=self.bounds[0],vmax=self.bounds[1], cmap= self.cmap,interpolation='none')
        ax.set_title('Image %d' % (ax.index))
...
def multiSliceViewerBad(volume, cmap = None):
    """
    dispaly that cycles through slices of a volume
    """
    Viewer(volume, cmap)
...
def genTomo2(tomoDataFolder):
    tomoImgs1=np.zeros([1,720,862,1022])
    print('Loading Images, Removing Negative Values, Applying Intensity Correction, Building Radiographs...')
    for x in np.arange(720):
        print('Image #: ' + str(x))
        #loads data
        tmp_img1=img.imread(tomoDataFolder+'/Riparia 25 high%0.4d.tif' %(x))
        #removes negative and applies intensity corrections
        tmp_img21=tomopy.misc.corr.remove_neg(tmp_img1 - img.imread(tomoDataFolder + '/di1.tif'), val=0.0,)*1

        #normalizes in some way idk
        tomoImgs1[0,x,:,:]=tomopy.prep.normalize.minus_log(tmp_img21 / img.imread(tomoDataFolder + '/ff1.tif'))
            
    return np.swapaxes(tomoImgs1,1,2)

def previous_slice(ax):
    volume = ax.volume
    ax.index = (ax.index - 1) % volume.shape[0]  # wrap around using %
#    fig = plt.figure(1)k
    bounds = _guessVBounds(volume[ax.index])
    ax.imshow(volume[ax.index], vmin=bounds[0],vmax=bounds[1], cmap='gist_earth',interpolation='none')
    ax.set_title('Image %d' % (ax.index))
#    plot_ff_data(ax)
    ax.set_title('Image %d' % (ax.index))
    #plot_ff_data(ax)
...
def genDark(tdf_data_folder, tdf_fold_start = None,num2skip = None,
    ...
    #finding the meta data
        ...
        theta = metaData[:,1]
        firstImage = os.listdir(tdf_data_folder)[0]
        ...
        for x in range(0, len(theta)-1):
            if(theta[x] > 0):
                tdf_stack[x,:,:]=img.imread(tdf_data_folder+'nf_%0.6d.tif' %(int(firstImage) + x + 2))
                #image_stack[x,:,:]=np.flipud(tmp_img>threshold)
            else:
                counter += 1
...
def genBright(tbf_data_folder, tdf, tbf_fold_start = None, num2skip = None, tbf_num_imgs = None):
    if(tbf_fold_start is not None and num2skip is not None and tbf_num_imgs is not None):
        ...
        theta = metaData[:,1]
        firstImage = os.listdir(tbf_data_folder)[0]
        firstImage = firstImage[3:firstImage.index('.tif')]
...
def genTomo(tomoDataFolder, tdf, tbf, intCorr = None, tomo_fold_start = None,num2skip = None, tomo_num_imgs = None):
    if(tomo_fold_start is None):
        tomo_fold_start = os.listdir(tomoDataFolder)[0]
    ...
    tomoImgs=np.zeros([1,tomo_num_imgs,2048,2048])
    ...
    for ii in np.arange(num_tomoImgs):
        ...
        #removes negative and applies intensity corrections
        tmp_img2=tomopy.misc.corr.remove_neg(tmp_img-tdf, val=0.0,)*intCorr[ii]

        #normalizes in some way idk
        tomoImgs[0,ii,:,:]=tomopy.prep.normalize.minus_log(tmp_img2/tbf)
    print("complete!")
...
def getIntCorr(tomoDataFolder):
    ...
    -------
    ndarray
        contains intencity correction values for tomographic data
